...
def _():
    ...
    return (FEATURE_NAMES, LogisticRegression, Pipeline, StandardScaler, clone,
            loguniform, mo, nested_loso_f1, np, plt, save_report, splits, to_report)
...
def _(mo, report):
    mo.md(
        ...
        f"Saved to `reports/logreg_nested.json`."
    )
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""### Coefficients (fit on all data)""")
    return
